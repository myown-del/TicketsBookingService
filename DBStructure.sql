...
create table if not exists bt.actions(
    id bigint default nextval('bt.act_seq'::regclass),
    ...
    constraint act_invalid_type_check
    check (type in ('play', 'movie'))
    )
    without oids;
...
create table if not exists bt.users(
    ...
    --
    constraint user_pk primary key (id)
>>>>>>> 67a30cab67cb6924edc6a08fa49b5565d683dfae
    )
    without oids ;
...
begin alter table bt.sessions add constraint sess_act_fk foreign key (action_id) references bt.actions(id); exception when duplicate_object then null; end;
begin alter table bt.sessions add constraint sess_hall_fk foreign key (hall_id) references bt.halls(id); exception when duplicate_object then null; end;
>>>>>>> 67a30cab67cb6924edc6a08fa49b5565d683dfae
